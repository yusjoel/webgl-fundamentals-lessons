...
function main() {
    ...
    function drawScene() {
        ...
        for (let i in curves) {
            ...
            // uColor1-3 ["#c2cb88", "#c1c490", "#adc7d0"]
            uniform = curve.shader.uniforms['uColor1'];
            gl.uniform3f(uniform.location, 194.0 / 255.0,203.0 / 255.0,136.0 / 255.0);
            uniform = curve.shader.uniforms['uColor2'];
            gl.uniform3f(uniform.location, 193.0 / 255.0,196.0 / 255.0,144.0 / 255.0);
            uniform = curve.shader.uniforms['uColor3'];
            gl.uniform3f(uniform.location, 173.0 / 255.0,199.0 / 255.0,208.0 / 255.0);

            // 提取颜色
            uniform = curve.shader.uniforms['uColorIsolate'];
            gl.uniform3f(uniform.location, color === 1 ? 1 : 0,color === 2 ? 1 : 0,color === 3 ? 1 : 0);

            // disabledCarColor #442825
            uniform = curve.shader.uniforms['uDisabledColor'];
            gl.uniform3f(uniform.location, 68.0 / 255.0,40.0 / 255.0,37.0 / 255.0);
            uniform = curve.shader.uniforms['uDisable'];
